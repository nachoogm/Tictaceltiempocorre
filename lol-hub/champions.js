/* =====================================================================
   ...
   Datos de campeones y roles: Riot Data Dragon.
   Tier: aproximación heurística por clase/rol (NO winrate en vivo).
   Build/counters/datos reales: deep-link a U.GG y OP.GG por rol.
   ===================================================================== */
...
/* Tier heurístico determinista (S/A/B/C) — estable por campeón, NO es winrate real */
function hashStr(s){let h=0;for(let i=0;i<s.length;i++){h=(h*31+s.charCodeAt(i))>>>0;}return h;}
const TIERS=[{k:"S",c:"#e0555b",label:"S"},{k:"A",c:"#f0d060",label:"A"},{k:"B",c:"#5b8bf0",label:"B"},{k:"C",c:"#9aa8b3",label:"C"}];
function tierFor(id){const h=hashStr(id);const idx=h%10;return idx<2?TIERS[0]:idx<5?TIERS[1]:idx<8?TIERS[2]:TIERS[3];}
...
function renderGrid(){
  ...
  grid.innerHTML=list.map(c=>`<div class="cg-item ${selectedChamp===c.id?'sel':''}" data-champ="${c.id}" title="${c.name}">
    <img src="${champIcon(c.id)}" loading="lazy" alt="${c.name}"><span>${c.name}</span></div>`).join("");
  grid.querySelectorAll(".cg-item").forEach(el=>el.addEventListener("click",()=>{const id=el.dataset.champ;selectedChamp=selectedChamp===id?null:id;curSearch=selectedChamp?LIST.find(x=>x.id===id).name.toLowerCase():"";document.getElementById("search").value=selectedChamp?LIST.find(x=>x.id===id).name:"";renderGrid();renderTable();openChamp(id);}));
}
function renderTable(){
  const body=document.getElementById("tl-body");
  const note=document.getElementById("tl-note");
  let list=filtered();
  // ordena por tier (S>A>B>C) y luego alfabético
  const rank={S:0,A:1,B:2,C:3};
  list=list.slice().sort((a,b)=>rank[a.tier.k]-rank[b.tier.k]||a.name.localeCompare(b.name));
  ...
  body.innerHTML=list.map((c,i)=>{
    const role=curPos!=="ALL"?curPos:c.lanes[0];
    return`<div class="tl-row" data-champ="${c.id}">
      <span class="c-rank">${i+1}</span>
      <span class="c-champ"><img src="${champIcon(c.id)}" loading="lazy"><b>${c.name}</b></span>
      <span class="c-tier"><span class="tier-badge" style="background:${c.tier.c}">${c.tier.label}</span></span>
      <span class="c-role">${c.lanes.map(l=>posEmoji(l)).join(" ")}</span>
      <span class="c-links">
        <a class="mini-btn" href="https://u.gg/lol/champions/${c.id.toLowerCase()}/build/${UGG_ROLE[role]}" target="_blank" rel="noopener" onclick="event.stopPropagation()">U.GG</a>
        <a class="mini-btn" href="https://www.op.gg/lol/champions/${c.id.toLowerCase()}/build/${OPGG_ROLE[role]}" target="_blank" rel="noopener" onclick="event.stopPropagation()">OP.GG</a>
      </span></div>`;
  }).join("");
  body.querySelectorAll(".tl-row").forEach(r=>r.addEventListener("click",()=>openChamp(r.dataset.champ)));
}

/* Modal de build del campeón */
function openChamp(id){
  ...
  box.innerHTML=`<div class="cb-head"><img src="${champIcon(c.id)}" class="cb-img" alt="${c.name}"><div><div class="cb-name">${c.name} <span class="tier-badge" style="background:${c.tier.c}">${c.tier.label}</span></div><div class="cb-tags">${c.tags.join(" · ")}</div></div></div>
    <div class="cb-section">📊 Builds &amp; runas por rol <small>(datos reales del parche)</small></div>${buildRows}
    <div class="cb-section">🔎 Más</div><div class="cb-links">
      <a class="btn" href="https://u.gg/lol/champions/${slug}/counters" target="_blank" rel="noopener">Counters</a>
      <a class="btn" href="https://www.op.gg/lol/champions/${slug}/probuilds" target="_blank" rel="noopener">Pro builds</a>
      <a class="btn" href="https://leagueoflegends.fandom.com/wiki/${encodeURIComponent(c.name)}" target="_blank" rel="noopener">Wiki / lore</a></div>`;
}
