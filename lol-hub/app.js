...
let CHAMPIONS = [];
let CHAMP_BY_KEY = {};   // numeric key -> {id,name}
async function loadDDragon(){
  try{ const vers = await fetch("https://ddragon.leagueoflegends.com/api/versions.json").then(r=>r.json()); DDRAGON_VER = vers[0]; }catch(e){}
  ...
  try{
    ...
    CHAMPIONS.forEach(c=> CHAMP_BY_KEY[c.key] = c);
  }catch(e){}
}
...
const TIER_COLORS = {IRON:"#7d7d7d",BRONZE:"#a5713d",SILVER:"#9aa8b3",GOLD:"#e6b84f",
  PLATINUM:"#4fd1c5",EMERALD:"#2ecc71",DIAMOND:"#5b8bf0",MASTER:"#b45cf0",GRANDMASTER:"#e0555b",CHALLENGER:"#f0d060"};
...
/* mini-badge de rango reutilizable */
function rankBadge(s, label){
  ...
  return `<div class="q-row">
    ...
    <span class="q-wr" style="color:${wrColor}">${s.winrate}%</span>
    <span class="q-det">${s.wins}V ${s.losses}D</span>
  </div>`;
}
...
function renderCardStats(full, d, raw){
  ...
  if(slot){
    slot.innerHTML =
      rankBadge(d.solo, "SoloQ") +
      rankBadge(d.flex, "Flex") +
      (d.level?`<div class="lvl-line">Nivel ${d.level}</div>`:"");
  }
  // maestrías
  const mslot = document.querySelector(`[data-mastery="${CSS.escape(full)}"]`);
  if(mslot){
    const champs = (d.topChamps||[]).map(c=>CHAMP_BY_KEY[c.championId]).filter(Boolean);
...
async function loadMatches(){
  if(!CONFIG.liveStats) return;
  const count = CONFIG.matchCount || 4;
  await Promise.all(CONFIG.players.map(async raw=>{
    const p = parseId(raw.riotId);
    const box = document.querySelector(`[data-matches="${CSS.escape(p.full)}"]`);
    if(!box) return;
    try{
      const res = await fetch(`/api/matches?riotId=${enc(p.full)}&platform=${R.uggPlatform}&count=${count}`);
      const d = await res.json();
      ...
      box.innerHTML = d.matches.map(m=>{
        const champ = CHAMP_BY_KEY[m.championId];
        const icon = champ ? champIcon(champ.id) : champIcon(m.championName);
        const kdaColor = m.deaths===0 ? "var(--win)" : (m.kda>=3?"var(--gold-bright)":"var(--text)");
        return `<div class="mh-row ${m.win?'win':'loss'}">
          <img class="mh-champ" src="${icon}" onerror="this.style.visibility='hidden'" title="${m.championName}">
          <div class="mh-mid">
            <div class="mh-top">
              <span class="mh-res">${m.win?'Victoria':'Derrota'}</span>
              <span class="mh-q">${m.queue}</span>
            </div>
            <div class="mh-kda">
              <b style="color:${kdaColor}">${m.kills}/${m.deaths}/${m.assists}</b>
              <span class="mh-ratio">${m.kda} KDA</span>
              <span class="mh-cs">${m.cs} CS</span>
            </div>
          </div>
      ...
      box.innerHTML = `<div class="mh-empty">Historial no disponible</div>`;
    }
  }));
}
...
function renderLeaderboard(){
  ...
  const rows = CONFIG.players.map(raw=>{
    const p = parseId(raw.riotId); const d = STATS[p.full] || {};
  ...
  box.innerHTML = rows.map((row,i)=>{
    ...
    return `<div class="lb-row top${i+1}">
      ...
      <div class="lb-rank"><div class="r" style="color:${color}">${rankTxt}</div><div class="lp">${lpTxt}</div></div>
    </div>`;
  }).join("");
}
...
function buildCards(){
  ...
  CONFIG.players.forEach(raw=>{
    ...
    card.innerHTML = `
      ...
      ${CONFIG.liveStats?`<div class="mh-head">📜 Últimas partidas</div><div class="match-history" data-matches="${p.full}"><div class="mh-empty">Cargando historial…</div></div>`:""}
      <div class="links">
