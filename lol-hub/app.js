...
/* ---------- Datos EN VIVO vía Azure Function ---------- */
const TIER_COLORS = {
  IRON:"#7d7d7d", BRONZE:"#a5713d", SILVER:"#9aa8b3", GOLD:"#e6b84f",
  PLATINUM:"#4fd1c5", EMERALD:"#2ecc71", DIAMOND:"#5b8bf0",
  MASTER:"#b45cf0", GRANDMASTER:"#e0555b", CHALLENGER:"#f0d060"
};
const TIER_ES = {
...
async function loadLiveStats(){
  ...
  CONFIG.players.forEach(async raw=>{
    const p = parseId(raw.riotId);
    const slot = document.querySelector(`[data-live="${CSS.escape(p.full)}"]`);
    if(!slot) return;
    try{
      const url = `/api/summoner?riotId=${enc(p.full)}&platform=${R.uggPlatform}`;
      const res = await fetch(url);
      if(!res.ok) throw new Error(res.status);
      const d = await res.json();
      renderRank(slot, d);
    }catch(e){
      slot.innerHTML = `<span class="rank-none">Stats no disponibles</span>`;
    }
  });
}

function renderRank(slot, d){
  if(d.error){ slot.innerHTML = `<span class="rank-none">${d.error}</span>`; return; }
  const s = d.solo;
  if(!s){
  ...
  const tierEs = TIER_ES[s.tier] || s.tier;
  const wrColor = s.winrate >= 50 ? "var(--win)" : "var(--loss)";
  slot.innerHTML = `
    <div class="rank-line">
      <span class="rank-badge" style="color:${color};border-color:${color}">
        ${tierEs} ${s.rank} · ${s.lp} LP
      </span>
