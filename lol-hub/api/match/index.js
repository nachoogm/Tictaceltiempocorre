...
module.exports=async function(context,req){
  ...
  try{
    ...
    const parts=(info.participants||[]).map(p=>({riotId:(p.riotIdGameName?`${p.riotIdGameName}#${p.riotIdTagline}`:(p.summonerName||"—")),championName:p.championName,championId:p.championId,champLevel:p.champLevel,teamId:p.teamId,position:p.teamPosition||"",kills:p.kills,deaths:p.deaths,assists:p.assists,kda:p.deaths===0?(p.kills+p.assists):+(((p.kills+p.assists)/p.deaths).toFixed(1)),cs:(p.totalMinionsKilled||0)+(p.neutralMinionsKilled||0),gold:p.goldEarned||0,damage:p.totalDamageDealtToChampions||0,items:[p.item0,p.item1,p.item2,p.item3,p.item4,p.item5,p.item6],spell1:p.summoner1Id,spell2:p.summoner2Id,win:p.win}));
    const teams=(info.teams||[]).map(t=>({teamId:t.teamId,win:t.win,bans:(t.bans||[]).map(b=>b.championId).filter(c=>c>0),objectives:{baron:t.objectives?.baron?.kills||0,dragon:t.objectives?.dragon?.kills||0,tower:t.objectives?.tower?.kills||0}}));
    const data={matchId,queue:QUEUE[info.queueId]||"Partida",durationMin:Math.round((info.gameDuration||0)/60),when:info.gameEndTimestamp||info.gameCreation||null,participants:parts,teams};
