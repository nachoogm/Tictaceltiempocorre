/* Azure Function: /api/matches — historial resumido (match-v5)
   Optimizado: caché PUUID + caché lista + partidas inmutables + reintento 429 */
...
const LIST_CACHE=new Map(); const MATCH_CACHE=new Map(); const PUUID_CACHE=new Map();
const LIST_TTL=5*60*1000; const PUUID_TTL=6*60*60*1000;
...
async function getPuuid(acc,gameName,tagLine,key){
  ...
  PUUID_CACHE.set(ck,{t:Date.now(),puuid:a.puuid}); return a.puuid;
}
module.exports=async function(context,req){
  ...
  try{
    ...
    for(const id of ids){
      ...
      if(!m){try{m=await riotGet(`https://${acc}.api.riotgames.com/lol/match/v5/matches/${id}`,key);MATCH_CACHE.set(id,m);}catch(err){continue;}}
      const me=(m.info.participants||[]).find(p=>p.puuid===puuid); if(!me)continue;
      matches.push({matchId:id,platform,championName:me.championName,championId:me.championId,win:me.win,kills:me.kills,deaths:me.deaths,assists:me.assists,kda:me.deaths===0?(me.kills+me.assists):+(((me.kills+me.assists)/me.deaths).toFixed(1)),cs:(me.totalMinionsKilled||0)+(me.neutralMinionsKilled||0),position:me.teamPosition||"",queue:QUEUE[m.info.queueId]||"Partida",durationMin:Math.round((m.info.gameDuration||0)/60),when:m.info.gameEndTimestamp||m.info.gameCreation||null});
    }
