...
const LIST_CACHE=new Map(); const MATCH_CACHE=new Map(); const LIST_TTL=5*60*1000;
...
module.exports = async function (context, req) {
  ...
  try{
    ...
    const puuid=account.puuid; const lk=`${acc}:${puuid}:${count}`; let ids; const lh=LIST_CACHE.get(lk);
    if(lh&&(Date.now()-lh.t)<LIST_TTL){ ids=lh.ids; }
    else{ ids=await riotGet(`https://${acc}.api.riotgames.com/lol/match/v5/matches/by-puuid/${puuid}/ids?start=0&count=${count}`,key); LIST_CACHE.set(lk,{t:Date.now(),ids}); }
    const matches=[];
    for(const id of ids){
      ...
      matches.push({ matchId:id, platform, championName:me.championName, championId:me.championId, win:me.win,
        ...
        queue:QUEUE[m.info.queueId]||"Partida", durationMin:Math.round((m.info.gameDuration||0)/60),
    ...
    return send(s,{ error:e.status===404?"Jugador no encontrado.":e.status===429?"Límite de la Riot API alcanzado.":(e.status===401||e.status===403)?"API key inválida o caducada.":"Error consultando la Riot API.", riotStatus:e.status||null, riotId });
  }
};
