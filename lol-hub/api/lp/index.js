/* =====================================================================
   ...
   ---------------------------------------------------------------------
   GET  /api/lp?days=21   -> devuelve snapshots de todos (últimos N días)
   POST /api/lp {name, lp, abs, tier, rank}  -> guarda/actualiza el de HOY
   ---------------------------------------------------------------------
   Requiere env var TABLES_CONNECTION_STRING (la connection string del
   Storage Account). Si no está, responde 503 y el front usa localStorage.
   Tabla: "lphistory"  ·  PartitionKey=nombre  ·  RowKey=fecha (YYYY-MM-DD)
   ===================================================================== */
...
async function getClient(){
  ...
  try{
    ...
    _client = TableClient.fromConnectionString(conn, TABLE_NAME, { allowInsecureConnection:false });
    await _client.createTable().catch(()=>{}); // crea la tabla si no existe (idempotente)
    return _client;
  }catch(e){ return null; }
}
...
module.exports = async function (context, req) {
  ...
  try{
    if((req.method||"GET").toUpperCase()==="POST"){
      ...
      return send(200, { ok:true, saved:{name, date:entity.rowKey} });
    }

    // GET -> devolver últimos N días de todos
    ...
    const out = {}; // { name: [ {date,lp,abs,tier,rank} ... ] }
    ...
    return send(500, { error:"Error accediendo a Table Storage.", detail:String(e&&e.message||e).slice(0,200) });
  }
};
