/* =====================================================================
   ...
   Devuelve rango, winrate y nivel de un jugador usando la Riot API.
   ---------------------------------------------------------------------
   Query params:
     riotId   = "Nombre#TAG"   (obligatorio)
     platform = "euw1"|"na1"|"kr"...   (obligatorio)
   Requiere la variable de entorno RIOT_API_KEY (Application Settings).
   ===================================================================== */

// Plataforma de juego -> clúster regional de account-v1
...
// Caché en memoria (reduce llamadas y evita 429). TTL 10 min.
const CACHE = new Map();
...
module.exports = async function (context, req) {
  ...
  if(!key){
    return send(500, { error:"RIOT_API_KEY no configurada en Azure (Application Settings)." });
  }
  ...
  try{
    // 1) Riot ID -> PUUID  (clúster regional)
