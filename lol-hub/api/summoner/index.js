/* /api/summoner — rango, nivel, maestrías, en-partida */
...
module.exports=async function(context,req){
  ...
  try{
    ...
    /* AHORRO DE RATE LIMIT: spectator y mastery solo si se piden explícitamente.
       Por defecto: summoner + league = 2 llamadas (antes eran 5). */
    const wantLive=(req.query.live||"")==="1";
    const wantMastery=(req.query.mastery||"")!=="0";
    const [summoner,entries,masteries,live]=await Promise.all([
      ...
      wantMastery?riotGet(`https://${platform}.api.riotgames.com/lol/champion-mastery/v4/champion-masteries/by-puuid/${puuid}/top?count=3`,key).catch(()=>[]):Promise.resolve([]),
      wantLive?riotGet(`https://${platform}.api.riotgames.com/lol/spectator/v5/active-games/by-summoner/${puuid}`,key).then(g=>({inGame:true,gameMode:g.gameMode,gameLength:g.gameLength})).catch(()=>({inGame:false})):Promise.resolve({inGame:false})
    ]);
