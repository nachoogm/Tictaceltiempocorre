...
async function loadLiveStats(onCard){
  ...
  await Promise.all(ACCOUNTS.map(async acc=>{const p=parseId(acc.riotId);
    try{const d=await cachedFetch(`/api/summoner?riotId=${enc(p.full)}&platform=${R.uggPlatform}`,4*60*1000);STATS[p.full]=d;if(d.solo){recordLp(p.name,d.solo);pushLpShared(p.name,d.solo);}if(onCard)onCard(p.full,d,acc);}
