/* =====================================================================
   common.js — núcleo compartido (ahora con PERSONAS + CUENTAS)
   ===================================================================== */
...
// lista plana de cuentas, cada una con .owner (nombre real) y .personIcon
const ACCOUNTS = PEOPLE.flatMap(p => (p.accounts||[]).map(a => ({...a, owner:p.person, personIcon:p.icon||a.main})));
// compatibilidad con código antiguo que usaba CONFIG.players
...
/* ---------- Control de rate limit (Riot: 20 req/s · 100 req/2min) ----------
   Cada llamada a NUESTRA api dispara varias a Riot por dentro:
     /api/summoner = 5   ·   /api/matches = 1+matchCount   ·   /api/live = 2
   Por eso además del limitador de concurrencia hay un ESPACIADO temporal
   que garantiza no superar ~12 llamadas a Riot por segundo (margen de seguridad). */
...
const apiLimit=pLimit(2);            // máx 2 peticiones simultáneas a nuestra API
const RIOT_SAFE_PER_SEC=12;          // presupuesto seguro (límite real 20)
let _lastSlot=0;
/* reserva "cost" llamadas y devuelve cuántos ms hay que esperar antes de lanzarlas */
function reserveSlot(cost){
  const now=Date.now();
  const gap=Math.ceil((cost/RIOT_SAFE_PER_SEC)*1000);
  const start=Math.max(now,_lastSlot);
  _lastSlot=start+gap;
  return start-now;
}
const sleep=ms=>new Promise(r=>setTimeout(r,ms));
/* coste en llamadas a Riot de cada endpoint nuestro */
function riotCost(url){
  if(url.startsWith("/api/summoner")){let c=2;            // summoner+league (base)
    if(!/mastery=0/.test(url))c++;                         // +mastery
    if(/live=1/.test(url))c++;                             // +spectator
    return c;}
  if(url.startsWith("/api/matches")){const m=url.match(/count=(\d+)/);return 1+(m?+m[1]:4);}
  if(url.startsWith("/api/live"))return 2;                // account+spectator
  if(url.startsWith("/api/match"))return 1;
  return 0;                                                // /api/lp y /api/scores no tocan Riot
}
async function cachedFetch(url,ttlMs){
  ...
  const cost=riotCost(url);
  const run=async()=>{
    if(cost>0){const wait=reserveSlot(cost);if(wait>0)await sleep(wait);}   // ← espaciado anti-429
    for(let i=0;i<3;i++){
      const r=await fetch(url);
      if(r.status===429&&i<2){await sleep(1500*(i+1));continue;}            // backoff progresivo
...
function multiSearchUrl(){const names=ACCOUNTS.map(a=>parseId(a.riotId).full);return `https://www.op.gg/multisearch/${R.pg}?summoners=${enc(names.join(","))}`;}
...
async function loadDDragon(){
  ...
  try{const it=await fetch(`https://ddragon.leagueoflegends.com/cdn/${DDRAGON_VER}/data/es_ES/item.json`).then(r=>r.json());
    ITEMS={};Object.entries(it.data).forEach(([id,v])=>{ITEMS[id]={name:v.name,gold:(v.gold&&v.gold.total)||0,plain:v.plaintext||"",desc:stripHtml(v.description),tags:v.tags||[],into:v.into||[],from:v.from||[]};});
  }catch(e){}
...
function recordLp(name,solo){
  if(!name||!solo)return;
  let hist={};try{hist=JSON.parse(localStorage.getItem(lpKey(name))||"{}");}catch(e){}
  hist[todayStr()]={lp:solo.lp,abs:absLp(solo),tier:solo.tier,rank:solo.rank};
  const keys=Object.keys(hist).sort();while(keys.length>90){delete hist[keys.shift()];}
  try{localStorage.setItem(lpKey(name),JSON.stringify(hist));}catch(e){}
}
...
let LP_SHARED_OK=false;
function pushLpShared(name,solo){
  if(!name||!solo)return;
  const flag="lh:lppush:"+name+":"+todayStr();
  ...
  fetch("/api/lp",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name,lp:solo.lp,abs:absLp(solo),tier:solo.tier,rank:solo.rank})})
    .then(r=>{if(r.ok){LP_SHARED_OK=true;try{localStorage.setItem(flag,"1");}catch(e){}}}).catch(()=>{});
}
async function getSharedLp(days){
  try{const r=await fetch(`/api/lp?days=${days||21}`);const d=await r.json();if(r.ok&&d.configured){LP_SHARED_OK=true;return d.players||{};}return null;}
  catch(e){return null;}
}

/* Stats en vivo — itera CUENTAS */
const STATS={};
async function loadLiveStats(onCard){
  if(!CONFIG.liveStats)return;
  await Promise.all(ACCOUNTS.map(async acc=>{const p=parseId(acc.riotId);
    try{const d=await cachedFetch(`/api/summoner?riotId=${enc(p.full)}&platform=${R.uggPlatform}&live=1`,8*60*1000);STATS[p.full]=d;if(d.solo){recordLp(p.name,d.solo);pushLpShared(p.name,d.solo);}if(onCard)onCard(p.full,d,acc);}
...
/* mejor cuenta (por SoloQ) de una persona */
function bestAccountOf(personName){
  const accs=ACCOUNTS.filter(a=>a.owner===personName);
  let best=null,bs=-2;
  accs.forEach(a=>{const d=STATS[parseId(a.riotId).full]||{};const s=rankScore(d.solo);if(s>bs){bs=s;best={acc:a,data:d};}});
  return best;
}
...
async function loadMatches(){
  ...
  await Promise.all(ACCOUNTS.map(async acc=>{
    const p=parseId(acc.riotId);const box=document.querySelector(`[data-matches="${CSS.escape(p.full)}"]`);
    try{const d=await cachedFetch(`/api/matches?riotId=${enc(p.full)}&platform=${R.uggPlatform}&count=${count}`,8*60*1000);
      MATCHDATA[p.full]=d.matches||[];
      const streakEl=document.querySelector(`[data-streak="${CSS.escape(p.full)}"]`);if(streakEl)streakEl.innerHTML=streakHtml(d.matches||[]);
      if(!box)return;
      if(!d.matches||!d.matches.length){box.innerHTML=`<div class="mh-empty">Sin partidas recientes</div>`;return;}
    ...
    }catch(e){if(box)box.innerHTML=`<div class="mh-empty">Historial no disponible</div>`;}
  }));
}
...
async function openMatch(matchId,platform){
  ...
  try{
    ...
    const rowP=p=>{const ch=champById(p.championId);const icon=ch?champIcon(ch.id):champIcon(p.championName);const items=p.items.filter(i=>i&&i>0).map(i=>`<img class="mm-item" data-item="${i}" src="${itemIcon(i)}" onerror="this.style.display='none'">`).join("");
      const nick=(p.riotId.split('#')[0]||'').toLowerCase();
      const mineAcc=ACCOUNTS.find(a=>parseId(a.riotId).name.toLowerCase()===nick);
      const label=mineAcc?`${p.riotId.split('#')[0]} <small style="color:var(--cyan)">(${mineAcc.owner})</small>`:p.riotId.split('#')[0];
      return`<div class="mm-row${mineAcc?' mine':''}"><div class="mm-c" title="${p.championName} · Nivel ${p.champLevel}"><img class="mm-champ" src="${icon}" onerror="this.style.visibility='hidden'"><span class="mm-lvl">${p.champLevel}</span></div><div class="mm-spells"><img src="${spellIcon(p.spell1)}" title="${spellName(p.spell1)}" onerror="this.style.display='none'"><img src="${spellIcon(p.spell2)}" title="${spellName(p.spell2)}" onerror="this.style.display='none'"></div><div class="mm-pinfo"><div class="mm-name" title="${p.riotId}">${label} ${multi(p)}</div><div class="mm-pos">${POS_ES[p.position]||''}</div></div><div class="mm-kda" title="Ratio ${p.kda}"><b>${p.kills}/${p.deaths}/${p.assists}</b><span>${p.kda} KDA</span></div><div class="mm-num" title="${p.cs} súbditos · ${p.csPerMin}/min"><b>${p.cs}</b><span>${p.csPerMin}/min</span></div><div class="mm-num" title="${p.damage.toLocaleString()} daño"><b>${(p.damage/1000).toFixed(1)}k</b><div class="mm-dmgbar"><i style="width:${p.damagePct}%"></i></div></div><div class="mm-num hide-s" title="${p.gold.toLocaleString()} oro"><b>${(p.gold/1000).toFixed(1)}k</b><span>oro</span></div><div class="mm-num hide-s" title="Visión ${p.vision} · wards ${p.wards}/${p.wardsKilled}"><b>${p.vision}</b><span>visión</span></div><div class="mm-items">${items}</div></div>`;};
    const banRow=t=>{const b=(t.bans||[]).map(id=>{const c=champByNum(id);return c?`<img src="${champIcon(c.id)}" title="Ban: ${c.name}">`:"";}).join("");return b?`<div class="mm-bans" title="Baneos">🚫 ${b}</div>`:"";};
    const teamBlock=(list,team,win,label)=>{const sum=list.reduce((a,p)=>({k:a.k+p.kills,d:a.d+p.deaths,as:a.as+p.assists,g:a.g+p.gold}),{k:0,d:0,as:0,g:0});return`<div class="mm-team"><div class="mm-team-head ${win?'win':'loss'}"><span>${win?'🏆 Victoria':'💀 Derrota'} · ${label}</span><span class="mm-team-sum" title="Totales">${sum.k}/${sum.d}/${sum.as} · ${(sum.g/1000).toFixed(1)}k oro</span></div><div class="mm-obj">${objIcons(team.objectives||{})}</div>${banRow(team)}${header}${list.map(rowP).join("")}</div>`;};
    box.innerHTML=`<div class="mm-head"><div class="mm-title">${d.queue}</div><div class="mm-meta">${d.durationMin} min · ${timeAgo(d.when)}</div></div>${teamBlock(blue,tBlue,blueWin,"Equipo Azul")}${teamBlock(red,tRed,!blueWin,"Equipo Rojo")}<div class="mm-foot">Pasa el ratón sobre cada dato u objeto para ver qué es · Riot API</div>`;
...
function injectNav(active){
  const links=[["index.html","🏠 Inicio"],["ranking.html","🏆 Ranking"],["lp.html","📈 Evolución"],["live.html","🔴 En vivo"],["champions.html","🧙 Campeones"],["loldle.html","🎯 LoLdle"],["arena.html","🎮 Entreno"]];
