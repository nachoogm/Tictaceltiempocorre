...
function songUrl(s){return "music/"+encodeURIComponent(s.file);}
...
function initPlayer(){
  ...
  const st=pState();
  PBAR=document.createElement("div");PBAR.id="mini-player";PBAR.className="mplayer"+(st.playing||st.time>0?"":" idle");
  PBAR.innerHTML=`
  ...
  function load(idx,time,play){
    ...
    $("mp-art").src=champIcon(safeIcon(s.icon||"Poro"));
  ...
  function setPlayIcon(p){$("mp-play").textContent=p?"⏸":"▶";}
  window.playSong=(idx)=>{const s=pState();if(s.idx===idx&&!AUDIO.paused){AUDIO.pause();setPlayIcon(false);return;}load(idx,0,true);};
  ...
  AUDIO.addEventListener("timeupdate",()=>{
    if(AUDIO.duration){$("mp-seek").value=(AUDIO.currentTime/AUDIO.duration)*100;$("mp-cur").textContent=fmtTime(AUDIO.currentTime);$("mp-dur").textContent=fmtTime(AUDIO.duration);}
  ...
  else{const s=SONGS[0];$("mp-title").textContent=s.title;$("mp-artist").textContent=s.artist||"";$("mp-art").src=champIcon(safeIcon(s.icon||"Poro"));}
  // si el navegador bloqueó el autoplay, el primer clic en la web lo reanuda
