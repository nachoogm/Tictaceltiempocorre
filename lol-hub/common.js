/* =====================================================================
   ...
   · Limitador de concurrencia (evita ráfagas > límite Riot)
   · Caché navegador (localStorage) con TTL + reintento 429
   · Navbar responsive, DDragon, stats, historial, modal partida enriquecido
   ===================================================================== */
...
/* Limitador: máx 3 peticiones /api simultáneas */
...
async function loadDDragon(){
  ...
  try{const d=await fetch(`https://ddragon.leagueoflegends.com/cdn/${DDRAGON_VER}/data/es_ES/champion.json`).then(r=>r.json());
    CHAMPIONS=Object.values(d.data).map(c=>({id:c.id,name:c.name,key:c.key,tags:c.tags||[],partype:c.partype,info:c.info||{},stats:c.stats||{},title:c.title,blurb:c.blurb}));
...
/* Stats en vivo (caché navegador 4 min) */
const STATS={};
...
function objIcons(o){const bits=[];if(o.baron)bits.push(`🐉 Barón ${o.baron}`);if(o.dragon)bits.push(`🔥 Dragón ${o.dragon}`);if(o.herald)bits.push(`👁️ Heraldo ${o.herald}`);if(o.tower)bits.push(`🏰 Torres ${o.tower}`);return bits.join(" · ");}
async function openMatch(matchId,platform){
  ...
  try{
    ...
    const rowP=p=>{const ch=champById(p.championId);const icon=ch?champIcon(ch.id):champIcon(p.championName);const items=p.items.filter(i=>i&&i>0).map(i=>`<img class="mm-item" src="${itemIcon(i)}" onerror="this.style.display='none'">`).join("");const mine=CONFIG.players.some(pl=>parseId(pl.riotId).name.toLowerCase()===(p.riotId.split('#')[0]||'').toLowerCase());
      return`<div class="mm-row${mine?' mine':''}">
        <div class="mm-c"><img class="mm-champ" src="${icon}" onerror="this.style.visibility='hidden'" title="${p.championName}"><span class="mm-lvl">${p.champLevel}</span></div>
        ...
        <div class="mm-num"><b>${p.cs}</b><span>${p.csPerMin}/min</span></div>
        <div class="mm-num"><b>${(p.damage/1000).toFixed(1)}k</b><div class="mm-dmgbar"><i style="width:${p.damagePct}%"></i></div></div>
        <div class="mm-num hide-s"><b>${(p.gold/1000).toFixed(1)}k</b><span>oro</span></div>
        <div class="mm-num hide-s"><b>${p.vision}</b><span>visión</span></div>
        <div class="mm-items">${items}</div></div>`;};
    const banRow=(t)=>{const b=(t.bans||[]).map(id=>{const c=CHAMP_BY_ID_byKeyNum(id);return c?`<img src="${champIcon(c.id)}" title="Ban: ${c.name}">`:"";}).join("");return b?`<div class="mm-bans">🚫 ${b}</div>`:"";};
    const teamBlock=(list,team,win,label)=>{const sum=list.reduce((a,p)=>({k:a.k+p.kills,d:a.d+p.deaths,as:a.as+p.assists,g:a.g+p.gold}),{k:0,d:0,as:0,g:0});return`<div class="mm-team"><div class="mm-team-head ${win?'win':'loss'}"><span>${win?'🏆 Victoria':'💀 Derrota'} · ${label}</span><span class="mm-team-sum">${sum.k}/${sum.d}/${sum.as} · ${(sum.g/1000).toFixed(1)}k oro</span></div><div class="mm-obj">${objIcons(team.objectives||{})}</div>${banRow(team)}${list.map(rowP).join("")}</div>`;};
    box.innerHTML=`<div class="mm-head"><div class="mm-title">${d.queue}</div><div class="mm-meta">${d.durationMin} min · ${timeAgo(d.when)}</div></div>${teamBlock(blue,tBlue,blueWin,"Equipo Azul")}${teamBlock(red,tRed,!blueWin,"Equipo Rojo")}<div class="mm-foot">Datos oficiales de la Riot API (match-v5)</div>`;
...
function CHAMP_BY_ID_byKeyNum(num){return CHAMPIONS.find(c=>+c.key===+num);}
...
/* Navbar responsive (con botón hamburguesa en móvil) */
