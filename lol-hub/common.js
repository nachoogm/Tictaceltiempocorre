/* =====================================================================
   common.js — compartido por todas las páginas (navbar, DDragon, stats,
   historial, modal de partida). Cárgalo ANTES del script de cada página.
   ===================================================================== */
...
async function loadDDragon(){
  ...
  try{const d=await fetch(`https://ddragon.leagueoflegends.com/cdn/${DDRAGON_VER}/data/es_ES/champion.json`).then(r=>r.json());
    CHAMPIONS=Object.values(d.data).map(c=>({id:c.id,name:c.name,key:c.key,tags:c.tags||[]}));
...
async function loadLiveStats(onCard){
  ...
  await Promise.all(CONFIG.players.map(async raw=>{
    ...
    try{const res=await fetch(`/api/summoner?riotId=${enc(p.full)}&platform=${R.uggPlatform}`);const d=await res.json();if(!res.ok)throw d;STATS[p.full]=d;if(onCard)onCard(p.full,d,raw);}
...
/* ---------- Modal detalle de partida (DELEGACIÓN DE EVENTOS = clic robusto) ---------- */
...
function ensureModal(){
  ...
  const d=document.createElement("div"); d.className="modal-overlay"; d.id="match-modal";
  d.innerHTML=`<div class="modal"><button class="modal-close" data-close-modal>✕</button><div id="modal-content"></div></div>`;
...
async function openMatch(matchId,platform){
  ...
  }catch(e){box.innerHTML=`<div class="mh-empty">No se pudo cargar la partida 😕<br><small>${(e&&e.error)||'¿Desplegaste la carpeta /api/match?'}</small></div>`;}
}
...
// Delegación global: funciona aunque las filas se creen después
document.addEventListener("click",e=>{
  const row=e.target.closest("[data-match]");
  if(row){openMatch(row.getAttribute("data-match"),row.getAttribute("data-platform"));return;}
  if(e.target.id==="match-modal"||e.target.hasAttribute("data-close-modal"))closeMatch();
});
document.addEventListener("keydown",e=>{
  if(e.key==="Escape")closeMatch();
  if(e.key==="Enter"){const row=e.target.closest&&e.target.closest("[data-match]");if(row)openMatch(row.getAttribute("data-match"),row.getAttribute("data-platform"));}
});
...
function injectNav(active){
  const links=[["index.html","🏠 Inicio"],["ranking.html","🏆 Power Ranking"],["tools.html","⚔️ Herramientas"],["champions.html","🧙 Campeones"]];
  const nav=document.createElement("nav"); nav.className="navbar";
  nav.innerHTML=`<div class="nav-inner">
    <a class="nav-brand" href="index.html">⏳ <span>${CONFIG.groupName||"LoL Hub"}</span></a>
    <div class="nav-links">${links.map(([h,t])=>`<a href="${h}" class="${h===active?'active':''}">${t}</a>`).join("")}</div>
    <a class="nav-region" href="${multiSearchUrl()}" target="_blank" rel="noopener">${region.toUpperCase()} · OP.GG</a>
  </div>`;
