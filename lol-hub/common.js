...
async function loadDDragon(){
  ...
  try{const it=await fetch(`https://ddragon.leagueoflegends.com/cdn/${DDRAGON_VER}/data/es_ES/item.json`).then(r=>r.json());
    ITEMS={};Object.entries(it.data).forEach(([id,v])=>{ITEMS[id]={name:v.name,gold:(v.gold&&v.gold.total)||0,plain:v.plaintext||"",desc:stripHtml(v.description),tags:v.tags||[],into:v.into||[],from:v.from||[],depth:v.depth||1,boots:(v.tags||[]).includes("Boots"),mythic:/mítico|mythic/i.test(v.description||"")};});
  }catch(e){}
...
/* ---------- Snapshots de LP (localStorage) ---------- */
function todayStr(){const d=new Date();return d.getFullYear()+"-"+String(d.getMonth()+1).padStart(2,"0")+"-"+String(d.getDate()).padStart(2,"0");}
function lpKey(name){return "lh:lp:"+name;}
// valor absoluto de LP acumulado para poder comparar entre divisiones
...
/* ---------- LP COMPARTIDO (Azure Table Storage vía /api/lp) ---------- */
let LP_SHARED_OK=false; // se pone true si el backend responde configurado
// envía el snapshot de hoy al backend (1 vez al día por jugador, fire-and-forget)
function pushLpShared(name,solo){
  if(!name||!solo)return;
  const flag="lh:lppush:"+name+":"+todayStr();
  if(localStorage.getItem(flag))return; // ya enviado hoy
...
// lee el histórico compartido de todos; devuelve {name:[{date,lp,abs,tier,rank}]} o null si no configurado
...
/* Stats en vivo (+ registra snapshot de LP) */
const STATS={};
async function loadLiveStats(onCard){
  ...
  await Promise.all(CONFIG.players.map(async raw=>{const p=parseId(raw.riotId);
    try{const d=await cachedFetch(`/api/summoner?riotId=${enc(p.full)}&platform=${R.uggPlatform}`,4*60*1000);STATS[p.full]=d;if(d.solo){recordLp(p.name,d.solo);pushLpShared(p.name,d.solo);}if(onCard)onCard(p.full,d,raw);}
...
async function loadMatches(){
  ...
  await Promise.all(CONFIG.players.map(async raw=>{
    const p=parseId(raw.riotId);const box=document.querySelector(`[data-matches="${CSS.escape(p.full)}"]`);
...
/* Modal detalle partida (con tooltips objetos) */
...
async function openMatch(matchId,platform){
  ...
  try{
    ...
    const rowP=p=>{const ch=champById(p.championId);const icon=ch?champIcon(ch.id):champIcon(p.championName);const items=p.items.filter(i=>i&&i>0).map(i=>`<img class="mm-item" data-item="${i}" src="${itemIcon(i)}" onerror="this.style.display='none'">`).join("");const mine=CONFIG.players.some(pl=>parseId(pl.riotId).name.toLowerCase()===(p.riotId.split('#')[0]||'').toLowerCase());
      return`<div class="mm-row${mine?' mine':''}"><div class="mm-c" title="${p.championName} · Nivel ${p.champLevel}"><img class="mm-champ" src="${icon}" onerror="this.style.visibility='hidden'"><span class="mm-lvl">${p.champLevel}</span></div><div class="mm-spells"><img src="${spellIcon(p.spell1)}" title="${spellName(p.spell1)}" onerror="this.style.display='none'"><img src="${spellIcon(p.spell2)}" title="${spellName(p.spell2)}" onerror="this.style.display='none'"></div><div class="mm-pinfo"><div class="mm-name" title="${p.riotId}">${p.riotId.split('#')[0]} ${multi(p)}</div><div class="mm-pos">${POS_ES[p.position]||''}</div></div><div class="mm-kda" title="Ratio ${p.kda}"><b>${p.kills}/${p.deaths}/${p.assists}</b><span>${p.kda} KDA</span></div><div class="mm-num" title="${p.cs} súbditos · ${p.csPerMin}/min"><b>${p.cs}</b><span>${p.csPerMin}/min</span></div><div class="mm-num" title="${p.damage.toLocaleString()} daño"><b>${(p.damage/1000).toFixed(1)}k</b><div class="mm-dmgbar"><i style="width:${p.damagePct}%"></i></div></div><div class="mm-num hide-s" title="${p.gold.toLocaleString()} oro"><b>${(p.gold/1000).toFixed(1)}k</b><span>oro</span></div><div class="mm-num hide-s" title="Visión ${p.vision} · wards ${p.wards}/${p.wardsKilled}"><b>${p.vision}</b><span>visión</span></div><div class="mm-items">${items}</div></div>`;};
