/* loldle.js — Columnas como el LoLdle original:
   Campeón · Género · Posición(es) · Especie · Recurso · Tipo de gama · Región(es) · Año
   + 5 modos, selector de jugador y ranking diario compartido. */
(function injectCSS(){
  ...
  .ld-cell.g{background:#2fd074;color:#06210f}
  .ld-cell.y{background:#e6b84f;color:#2a1e00}
  .ld-cell.r{background:#ff6060;color:#2a0505}
  ...
  @media(max-width:760px){
    ...
    .ld-cell{min-height:52px;font-size:11px}
    ...
    .ld-cell{position:relative;padding-top:14px}
  }`;
  document.head.appendChild(s);
})();

/* ---------- Género (curado) ---------- */
const FEMALE=new Set(["Ahri","Akali","Anivia","Annie","Ashe","Ambessa","Aurora","Belveth","Briar","Caitlyn","Camille","Cassiopeia","Diana","Elise","Evelynn","Fiora","Gwen","Illaoi","Irelia","Janna","Jinx","Kaisa","Kalista","Karma","Katarina","Kayle","Leblanc","Leona","Lillia","Lissandra","Lulu","Lux","Mel","MissFortune","Morgana","Nami","Neeko","Nidalee","Nilah","Orianna","Poppy","Qiyana","Quinn","Rell","Renata","Riven","Samira","Sejuani","Senna","Seraphine","Shyvana","Sivir","Sona","Soraka","Syndra","Taliyah","Tristana","Vayne","Vex","Vi","Xayah","Yunara","Yuumi","Zeri","Zoe","Zyra","Nunu","Kindred","Naafiri"]);
const OTHERGEN=new Set(["Kindred","Maokai","Ivern","Zac","Skarner","Rammus","Khazix","Chogath","Belveth","Malzahar","Fiddlesticks","Amumu","AurelionSol","Naafiri","Velkoz","RekSai","Yuumi","Smolder","Bard","Nocturne","Shaco","Blitzcrank","Orianna","Galio","Malphite","Zyra","Anivia"]);
function genderOf(id){if(OTHERGEN.has(id))return "Otro";if(FEMALE.has(id))return "Femenino";return "Masculino";}
/* ---------- Recurso y tipo de gama (Data Dragon) ---------- */
...
/* compara listas: verde idénticas, amarillo si solapan, rojo si nada */
...
function submitGuess(id){
  ...
  const gGen=genderOf(g.id), tGen=genderOf(target.id);
  ...
  row.innerHTML=
    ...
    cell(gGen, gGen===tGen?"g":"r", "Género")+
    ...
    cell(gReg.join(", "), listState(gReg,tReg), "Región")+
