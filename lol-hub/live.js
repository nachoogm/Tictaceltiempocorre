/* live.js — partidas EN VIVO enriquecidas. Caché 20s + limitador + auto-refresh */
let liveTimer=null;
...
async function loadLive(){
  ...
  if(playing.length){
    html+=`<div class="live-cards">`+playing.map(({raw,p,d})=>{
      ...
      const row=x=>{const c=champByNum(x.championId);const icon=c?champIcon(c.id):"";return`<div class="lc-p ${x.isMe?'me':''}"><img class="lc-champ" src="${icon}" onerror="this.style.visibility='hidden'" title="${c?c.name:''}"><div class="lc-sp"><img src="${spellIcon(x.spell1)}" title="${spellName(x.spell1)}" onerror="this.style.display='none'"><img src="${spellIcon(x.spell2)}" title="${spellName(x.spell2)}" onerror="this.style.display='none'"></div>${x.keystone&&runeIcon(x.keystone)?`<img class="lc-rune" src="${runeIcon(x.keystone)}" title="${runeName(x.keystone)}" onerror="this.style.display='none'">`:`<span class="lc-rune-ph"></span>`}<span class="lc-n">${(x.riotId||'').split('#')[0]}</span></div>`;};
      ...
      return`<div class="lc-card"><div class="lc-banner" style="background-image:linear-gradient(180deg,rgba(7,11,20,.3),rgba(7,11,20,.94)),url('${splash}')"><div class="lc-top"><span class="lc-live">● EN VIVO</span><span class="lc-meta">${d.queue}${mapName?` · ${mapName}`:""} · ${d.minutes}′</span></div><div class="lc-hero"><img class="lc-hero-champ" src="${meChamp?champIcon(meChamp.id):''}" onerror="this.style.display='none'"><div class="lc-hero-info"><div class="lc-hero-name">${p.name}</div><div class="lc-hero-champname">${meChamp?meChamp.name:''} · ${raw.rol||''}</div><div class="lc-hero-rank" style="color:${rkColor};border-color:${rkColor}">${rk}</div></div></div></div>${bans?`<div class="lc-bans"><span>Baneos</span>${bans}</div>`:""}<div class="lc-teams"><div class="lc-team"><div class="lc-tt blue">🔵 Aliados</div>${blue.map(row).join("")}</div><div class="lc-vs">VS</div><div class="lc-team"><div class="lc-tt red">🔴 Enemigos</div>${red.map(row).join("")}</div></div><a class="btn primary lc-spectate" href="${urls(p).porofessor}" target="_blank" rel="noopener">👁️ Espectar en Porofessor</a></div>`;
    }).join("")+`</div>`;
  ...
  html+=`<div class="lv-status"><div class="lv-status-t">Estado de comprobación</div><div class="lv-status-grid">${results.map(({p,d})=>{const st=d.inGame?`<span class="st-live">🔴 En partida</span>`:(d.error?`<span class="st-err">⚠️ ${d.error}</span>`:`<span class="st-off">⚫ Libre</span>`);return `<div class="lv-st-row"><span>${p.name}</span>${st}</div>`;}).join("")}</div></div>`;
  grid.innerHTML=html;
}
