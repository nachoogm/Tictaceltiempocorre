...
let SCORES_OK=false;
async function submitScore(m,name,value){if(!name)return;try{const r=await fetch("/api/scores",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({board:boardOf(m),name,score:value,lower:MODES[m].lower})});if(r.ok)SCORES_OK=true;}catch(e){}saveLocalRank(m,name,value);}
...
async function fetchRank(m){try{const r=await fetch(`/api/scores?board=${boardOf(m)}`);const d=await r.json();if(r.ok&&d.configured){SCORES_OK=true;return d.scores||[];}}catch(e){}return getLocalRank(m);}
...
function setOverlay(title,desc,btn){const inner=$("ar-ov-inner");inner.innerHTML=`<div class="ar-ov-title">${title}</div><div class="ar-ov-desc">${desc}</div><button class="btn primary" id="ar-play">${btn}</button>`;$("ar-play").addEventListener("click",start);}
function setMode(m){if(running)stop();mode=m;document.querySelectorAll(".ar-modes .rk-tab").forEach(b=>b.classList.toggle("active",b.dataset.mode===m));
  const desc={aim:"Clica los campeones lo más rápido que puedas. ¡30 segundos!",dodge:"Mueve el ratón esquivando las habilidades. Aguanta lo máximo.",dodgeshoot:"Esquiva los MORADOS y clica los enemigos ROJOS. ¡A la vez!",reaction:"Espera a que la arena se ponga VERDE y clica. 5 rondas."}[m];
  setOverlay(MODES[m].label,desc,"▶ Jugar");arena().className="ar-arena";clearField();resetHud();showOverlay(true);showBest();}
function startAim(){running=true;score=0;misses=0;timeLeft=30;resetHud();showOverlay(false);clearField();arena().classList.add("aim-mode");
  const f=field();f._miss=e=>{if(e.target.closest(".ar-target"))return;misses++;updateAcc();};f.addEventListener("pointerdown",f._miss);
  tickTimer=setInterval(()=>{timeLeft-=0.1;if(timeLeft<=0){$("ar-time").textContent="0.0";endAim();return;}$("ar-time").textContent=timeLeft.toFixed(1);},100);scheduleSpawn();}
function scheduleSpawn(){if(!running)return;spawnChamp();spawnTimer=setTimeout(scheduleSpawn,Math.max(360,820-(30-timeLeft)*14));}
function spawnChamp(){const f=field();const {w,h}=areaSize();const size=Math.max(44,Math.min(72,w/12));
  const x=Math.max(0,Math.random()*Math.max(0,w-size)),y=Math.max(0,Math.random()*Math.max(0,h-size));
  const c=champPool[Math.floor(Math.random()*champPool.length)]||{id:"Poro"};
  const t=document.createElement("div");t.className="ar-target spawn";t.style.cssText=`position:absolute;left:${x}px;top:${y}px;width:${size}px;height:${size}px`;
...
function endAim(){running=false;clearInterval(tickTimer);clearTimeout(spawnTimer);const f=field();if(f._miss)f.removeEventListener("pointerdown",f._miss);clearField();
  const acc=(score+misses)?Math.round(score/(score+misses)*100):100;finish(score,`⏱️ ¡Tiempo!`,`Has acertado <b>${score}</b> campeones con <b>${acc}%</b> de precisión.`);}
let cv=null,ctx=null,W=0,H=0,mouse={x:0,y:0,in:false},shots=[],enemies=[],dodged=0,elapsed=0,spawnAcc=0,enemyAcc=0,lastT=0;
function buildCanvas(){clearField();const {w,h}=areaSize();cv=document.createElement("canvas");cv.className="ar-canvas";
  ...
  cv.addEventListener("pointermove",move);cv.addEventListener("touchmove",e=>{move(e);e.preventDefault();},{passive:false});cv.addEventListener("pointerleave",()=>mouse.in=false);
  cv.addEventListener("pointerdown",e=>{if(mode!=="dodgeshoot"||!running)return;const r=cv.getBoundingClientRect();const mx=(e.clientX-r.left)*(W/r.width),my=(e.clientY-r.top)*(H/r.height);
...
function startDodge(){running=true;shots=[];enemies=[];dodged=0;score=0;elapsed=0;spawnAcc=0;enemyAcc=0;lastT=performance.now();resetHud();showOverlay(false);
  arena().classList.add(mode==="dodgeshoot"?"shoot-mode":"dodge-mode");buildCanvas();mouse.x=W/2;mouse.y=H/2;mouse.in=true;raf=requestAnimationFrame(dodgeLoop);}
function spawnShot(){const side=Math.floor(Math.random()*4);let x,y;if(side===0){x=Math.random()*W;y=-20;}else if(side===1){x=W+20;y=Math.random()*H;}else if(side===2){x=Math.random()*W;y=H+20;}else{x=-20;y=Math.random()*H;}
  const tx=mouse.x+(Math.random()-0.5)*120,ty=mouse.y+(Math.random()-0.5)*120;const ang=Math.atan2(ty-y,tx-x);const speed=2.6+Math.min(4,elapsed/8);const r=9+Math.random()*7;
...
function dodgeLoop(t){if(!running||!ctx)return;const dt=Math.min(50,t-lastT);lastT=t;elapsed+=dt/1000;$("ar-time").textContent=elapsed.toFixed(1);
...
function endDodge(){running=false;cancelAnimationFrame(raf);const secs=elapsed.toFixed(1);clearField();
...
function startReaction(){running=true;rxRound=0;rxTimes=[];resetHud();showOverlay(false);clearField();arena().classList.add("rx-mode");
  const box=document.createElement("div");box.className="rx-box waiting";box.id="rx-box";box.style.cssText="position:absolute;inset:0;display:flex;align-items:center;justify-content:center";
  box.innerHTML=`<div class="rx-msg" id="rx-msg">Prepárate…</div>`;field().appendChild(box);
  box.addEventListener("pointerdown",()=>{if(rxState==="wait"){clearTimeout(rxTimeout);box.className="rx-box early";$("rx-msg").textContent="¡Demasiado pronto! 😅";rxState="idle";setTimeout(nextRound,900);}
  ...
  nextRound();}
function nextRound(){if(!running)return;const box=$("rx-box");if(!box)return;box.className="rx-box waiting";$("rx-msg").textContent=`Ronda ${rxRound+1}/5 · espera al VERDE…`;rxState="wait";
...
function endReaction(){running=false;const avg=Math.round(rxTimes.reduce((a,b)=>a+b,0)/rxTimes.length);const best=Math.min(...rxTimes);clearField();
  finish(avg,`⚡ ¡Terminado!`,`Media: <b>${avg} ms</b> · mejor: <b>${best} ms</b><br><small>${rxTimes.join(" · ")} ms</small>`);}
async function finish(value,title,html){const name=currentPlayer();const isRec=setLocalBest(mode,value);showBest();
  const extra=`${isRec?"<br>🏆 <b>¡Nuevo récord personal!</b>":""}${name?"":"<br><small>Elige tu nombre arriba para guardar la marca.</small>"}`;
  setOverlay(title,html+extra,"🔁 Otra vez");showOverlay(true);
  if(name){await submitScore(mode,name,value);await refreshRank();}}
function start(){if(!champPool.length)champPool=(typeof CHAMPIONS!=="undefined"&&CHAMPIONS.length)?CHAMPIONS:[{id:"Poro"}];
  if(mode==="aim")startAim();else if(mode==="reaction")startReaction();else startDodge();}
function stop(){running=false;clearInterval(tickTimer);clearTimeout(spawnTimer);clearTimeout(rxTimeout);if(raf)cancelAnimationFrame(raf);
  const f=$("ar-field");if(f&&f._miss)f.removeEventListener("pointerdown",f._miss);clearField();}
let rankView="aim";
async function refreshRank(){renderRank(await fetchRank(rankView));}
function renderRank(list){const body=$("ar-rank-body");if(!body)return;const M=MODES[rankView];
...
function bindRank(){document.querySelectorAll(".ar-rank-tabs .fb-btn").forEach(b=>b.addEventListener("click",async()=>{
  document.querySelectorAll(".ar-rank-tabs .fb-btn").forEach(x=>x.classList.remove("active"));b.classList.add("active");
  rankView=b.dataset.rk;$("ar-rank-body").innerHTML=`<div class="mh-empty">Cargando…</div>`;renderRank(await fetchRank(rankView));$("ar-rank-sub").textContent=subText();}));}
(async function(){
  ...
  await loadDDragon();champPool=CHAMPIONS.length?CHAMPIONS:[{id:"Poro"}];
