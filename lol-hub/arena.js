/* arena.js — 4 minijuegos + RANKING COMPARTIDO (Azure vía /api/scores).
   FIX: el overlay ya nunca se destruye; todo el juego vive en #ar-field. */
let mode="aim",running=false,raf=null,tickTimer=null,spawnTimer=null;
let score=0,misses=0,timeLeft=30,champPool=[];
const $=id=>document.getElementById(id);
const arena=()=>$("arena");
const field=()=>$("ar-field");          // <— campo de juego (se puede limpiar sin miedo)
...
function clearField(){const f=field();if(f)f.innerHTML="";cv=null;ctx=null;}
...
function setMode(m){
  ...
  arena().className="ar-arena";      // reset de clases (cursor)
  clearField();                       // limpia SOLO el campo, el overlay sobrevive
...
function startAim(){
  ...
  clearField();const a=arena();a.classList.add("aim-mode");
  const f=field();
...
function spawnChamp(){
  const f=field();const rect=f.getBoundingClientRect();
...
function buildCanvas(){
  clearField();
  cv=document.createElement("canvas");cv.className="ar-canvas";field().appendChild(cv);ctx=cv.getContext("2d");
...
function resizeCanvas(){if(!cv)return;const r=field().getBoundingClientRect();W=cv.width=r.width;H=cv.height=r.height;}
function startDodge(){
  ...
  buildCanvas();mouse.x=W/2;mouse.y=H/2;
...
function start(){
  if(!champPool.length)champPool=CHAMPIONS.length?CHAMPIONS:[{id:"Poro"}];
...
function stop(){
  ...
  const f=field();if(f&&f._miss)f.removeEventListener("pointerdown",f._miss);
  clearField();
}
...
(async function(){
  ...
  // el selector y los modos deben funcionar aunque DDragon tarde
